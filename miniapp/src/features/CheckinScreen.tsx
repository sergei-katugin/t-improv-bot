...
import { Alert, Button, Group, Loader, Modal, NumberInput, Paper, Progress, Stack, Text, TextInput, Title } from "@mantine/core";
import { api } from "../lib/api";
import { showNotification } from "../lib/notifications";

type Entry = { kind: "registration" | "manual"; id: number; name: string; booked: number; arrived: number };
type Arrivals = { id: number; title: string; mode: "counter" | "named"; reportEvery: number; arrived: number; unidentified: number; booked: number; remaining: number; percent: number; items: Entry[] };
...
export function CheckinScreen({ showId, canConfigure = false }: { showId: number; canConfigure?: boolean }) {
  ...
  async function perform(path: string, body: object, method = "POST") {
    if (busy) return;
    setBusy(true);
    try { await api(`/api/miniapp/shows/${showId}/checkin${path}`, { method, body: JSON.stringify({ expected: data?.unidentified ?? 0, ...body }) }); await load(); return true; }
  ...
  return <Stack>
    ...
    <Text c="dimmed">Отчёт в чат записей — каждые {data.reportEvery} человек. Без найденной записи: {data.unidentified ?? 0}. При ручном учёте процент приблизительный.</Text>
    ...
    {canConfigure && <>
      <Button variant="default" onClick={() => { setStep(data.reportEvery); setEditing(true); }}>Настроить отчёты входа</Button>
      ...
      <Modal zIndex={400} opened={editing} onClose={() => setEditing(false)} title="Настройки входа"><Stack><NumberInput label="Отчёт каждые N человек" min={1} max={100} value={step} onChange={(value) => setStep(Number(value))} /><Button loading={busy} onClick={() => void perform("/config", { mode: data.mode, reportEvery: step }, "PUT").then((saved) => { if (saved) setEditing(false); })}>Сохранить</Button></Stack></Modal>
    </>}
    <Button variant="default" onClick={() => setManualOpened(true)}>Не нашёл — указать вручную</Button>
    <Modal zIndex={400} opened={manualOpened} onClose={() => setManualOpened(false)} title="Пришли без найденной записи"><Stack><Text>Добавь только тех, кого не отметил по имени. Если запись найдётся позже, сначала убери их из ручного счётчика.</Text><Text>Учтено вручную: {data.unidentified ?? 0}</Text><Group grow>{[1, 2, 3].map((delta) => <Button key={delta} size="lg" disabled={busy} onClick={() => void perform("", { delta })}>+{delta}</Button>)}<Button variant="default" disabled={busy || !data.unidentified} onClick={() => void perform("", { delta: -1 })}>−1</Button></Group></Stack></Modal>
  </Stack>;
}
