...
export function newShowForm(): ShowFormValue {
  ...
  return { title: "", titleNewcomer: "", teamName: "", showDateLocal: local, location: "", locationUrl: "", city: "Лимасол", posterText: "", posterTextNewcomer: "", maxSeats: 50, maxGuests: 6, registrarUsername: "", checkinEnabled: true, checkinMode: "named", checkinReportEvery: 10, feedbackEnabled: true };
}
...
function formFromShow(show: Show): ShowFormValue {
  return {
    ...
    checkinEnabled: true, checkinMode: show.checkinMode ?? "named", checkinReportEvery: show.checkinReportEvery ?? 10, feedbackEnabled: show.feedbackEnabled ?? false,
  };
}
  ...
  >
    <form onSubmit={submit} className="show-form">
      <Stack gap="md">
        ...
        {<NumberInput label="Отчёт в чат каждые N человек" min={1} max={100} value={value.checkinReportEvery ?? 10} onChange={(next) => set("checkinReportEvery", Number(next))} />}
